...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Validate structure prediction submission zip file.")
    parser.add_argument("zip_file", type=str, help="Path to the structure_predictions.zip file")
    args = parser.parse_args()

    df = pd.read_csv("./pxr-challenge_structure_TEST_BLINDED.csv")

    is_valid, validation_errors = validate_structure_submission(args.zip_file, expected_ids=set(df["structure"]), expected_ligand_smiles=dict(zip(df["structure"], df["smiles"])))
    if is_valid:
        print("Submission is valid!")
    else:
        print("Submission is INVALID. Errors found:")
