...
echo "Evaluating ensembles for highest complex_iplddt..."
...
for ligand_dir in boltz_outputs/x*; do
    ...
    while IFS= read -r json_file; do
        ...
        current_score=$(grep '"complex_iplddt"' "$json_file" | awk -F':' '{print $2}' | tr -d ' ,')
        
        # Skip if grep failed to find the line
        [ -z "$current_score" ] && continue
        
        # Compare floating point numbers using awk
        is_better=$(awk -v curr="$current_score" -v best="$best_score" 'BEGIN {if (curr > best) print 1; else print 0}')
    ...
    if [ -n "$best_pdb" ] && [ -f "$best_pdb" ]; then
        ...
        echo "Selected top pose for $ligand_id -> $(basename "$best_pdb") (score: $best_score)"
    else
